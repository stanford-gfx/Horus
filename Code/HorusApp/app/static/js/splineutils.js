define(function() {
    ...
    **/
    ...
    //
    // @knots: array of arrays of [x, y, z, ...] points
    ...
    var simpleCatmullRomTangent  = function(p0,p1,p2,t0,t1,t2) { return (p2 - p0)/(t2 - t0); }
    var simpleCatmullRomTangent1  = function(p0,p1,t0,t1) { return (p1-p0)/(t1-t0); }

    var centripetalCatmullRomTangent  = function(p0,p1,p2,t0,t1,t2) { return (p1-p0)/(t1-t0) - (p2-p0)/(t2-t0) + (p2-p1)/(t2-t1); }
    var centripetalCatmullRomTangent1 = function(p0,p1,t0,t1) { return (p1-p0)/(t1-t0); }

    var catmullRomTangent = centripetalCatmullRomTangent;
    var catmullRomTangent1 = centripetalCatmullRomTangent1;

    // var catmullRomTangent = simpleCatmullRomTangent;
    // var catmullRomTangent1 = simpleCatmullRomTangent1;
    ...
    // @return: List of list of vector of constraints per segment [[x0(0), x0(1), x0'(0), x0'(1), ...], [...]]
    Spline.calcCatmullRomConstraints = function(X, T, degree, zeroEndpoints) {
        ...
        for (var dim = 0; dim < X.length; dim++) {
            ...
            for (var i = 0; i < Xd.length; i++) {
                if (i == 0)                { m1[i] = zeroEndpoints ? 0 : catmullRomTangent1(Xd[0], Xd[1], T[0], T[1]); }
                else if (i == Xd.length-1) { m1[i] = zeroEndpoints ? 0 : catmullRomTangent1(Xd[i-1], Xd[i], T[i-1], T[i]); }
    ...
    Spline.pythonToJSCoefficients = function(pythonCoefficients, numKnots) {
        var JSCoefficients = [];
        var objectLen = pythonCoefficients[0].length;
        for (var o = 0; o < objectLen; o++) {
            JSCoefficients.push([]);
            for (var v = 0; v < numKnots - 1; v++) {
                var numCoeffs = pythonCoefficients.length/(numKnots - 1);
                JSCoefficients[o].push(Vector.Zero(numCoeffs));
                for (var j = 0; j < numCoeffs; j++) {
                    var pyIndx = v * numCoeffs + j;
                    JSCoefficients[o][v].elements[j] = pythonCoefficients[pyIndx][o];
                }
            }
        }

        return JSCoefficients;
    }

    Spline.pytonToJSParamSpacing = function(pytonParamSpacing) {
        var JSParamSpacing = [];
        for (var i in pytonParamSpacing) {
            JSParamSpacing.push(pytonParamSpacing[i][0]);
        }
        return JSParamSpacing;
    },
